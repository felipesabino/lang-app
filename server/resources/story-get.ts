...
import { AudioSpeed, SpeechMark, Story, StoryStatusType } from './model/graphql-schema'
import { Story as StoryDb } from './model/story-dynamodb';
import { S3Client, GetObjectCommand } from '@aws-sdk/client-s3';
import { SpeechMarkType } from '@aws-sdk/client-polly';
...
export const handler = async (event: any): Promise<Story> => {
  try {
    ...
    const STORY_ID = event.arguments.story as string;
    const TEXT_KEY = `${STORY_ID}/text.txt`;
    const TRANSLATION_KEY = `${STORY_ID}/translation.txt`;
    const AUDIO_SLOW_FILE_KEY = `${STORY_ID}/audio-${AudioSpeed.Slow.toLocaleLowerCase()}.mp3`;
    const AUDIO_NORMAL_FILE_KEY = `${STORY_ID}/audio-${AudioSpeed.Normal.toLocaleLowerCase()}.mp3`;
    const SPEECH_SLOW_FILE_KEY = `${STORY_ID}/speech-marks-${AudioSpeed.Slow.toLocaleLowerCase()}.json`;
    const SPEECH_NORMAL_FILE_KEY = `${STORY_ID}/speech-marks-${AudioSpeed.Normal.toLocaleLowerCase()}.json`;
