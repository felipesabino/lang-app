...
import { AudioSpeed, SpeechMark, Story, StoryStatusType, QueryGetStoryByIdArgs } from "@langapp/graphql";
import { Story as StoryDb, translateConfig } from "../model";
...
export const handler = async (event: any): Promise<Story> => {
  try {
    ...
    if (!storyId) {
      throw new Error("storyId not defined");
    }
    ...
    const speechMarkNormalObject = await s3client.send(
      new GetObjectCommand({
    ...
    const speechMarkSlowObject = await s3client.send(
      new GetObjectCommand({
        Bucket: AUDIO_BUCKET_NAME,
    ...
    const speechMarksSlow: SpeechMark[] = JSON.parse(
      (await speechMarkSlowObject.Body?.transformToString("utf-8")) || "[]"
    );
    const speechMarksNormal: SpeechMark[] = JSON.parse(
      (await speechMarkNormalObject.Body?.transformToString("utf-8")) || "[]"
    );
