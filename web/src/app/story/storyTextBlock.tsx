        ...
        <p className={classNames({
          ...
          "underline": false,
            ...
            {paragraph.words.map((word, index) =>
              <span key={"w-" + index} className={classNames({
       ...
       {toRender.text.map((item, index) =>
          <div key={"p-" + index}>
            ...
            <details className="pb-2">
              <summary className="text-sm leading-6 text-slate-900font-semibold select-none hover:cursor-help">Translation</summary>
              <p className="mt-2 text-sm leading-6 text-slate-600 italic">
                {toRender.translation[index].text}
              </p>
            </details>
          </div>
        )}
        ...
        <div className={classNames({
          ...
          'hidden': textExplanationIsLoading,
            ...
            {textExplanation.explanation.split("\n").map((paragraph, index) =>
              <p className="pt-2" key={"exp-" + index}>{paragraph}</p>
            )}
      </div>
    </div>
  </div>
